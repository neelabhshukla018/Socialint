...
export async function getProfiles(clerkId) {
    ...
    const profilesWithSources = await Promise.all(profiles.map(async (p) => {
...
export async function setActiveProfile(id, clerkId) {
    const user = await db.orm.public.User.first({
        clerkId,
    });
    if (!user) {
