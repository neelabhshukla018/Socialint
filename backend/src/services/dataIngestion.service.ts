...
import { mlClient, SentimentResult, EmotionResult } from "./mlClient.service.js";
import { db } from "../prisma/db.js";

export type SocialPlatform = "X" | "INSTAGRAM" | "FACEBOOK" | "GITHUB" | "TELEGRAM" | "YOUTUBE" | "REDDIT";

/**
 * Normalized Social User Schema
 */
...
/**
 * Normalized Interaction Schema
 */
export interface NormalizedInteraction {
  id: string;
  platform: SocialPlatform;
  type: "REPLY" | "MENTION" | "QUOTE" | "RETWEET" | "SHARE";
  sourceUserId: string;
  targetUserId: string;
  postId?: string;
  timestamp: string;
...
/**
 * Ingestion Result with ML enrichments
 */
export interface IngestedPostResult {
...
export class DataIngestionService {
  ...
  normalizePost(raw: {
    ...
    const platform = (raw.platform.toUpperCase() as SocialPlatform) || "INSTAGRAM";
  ...
  async ingestAndSaveBatch(
    ...
    for (const item of enriched) {
      const p = item.post;
      const sentLabel = (item.sentiment.label as "POSITIVE" | "NEGATIVE" | "NEUTRAL") || "NEUTRAL";
