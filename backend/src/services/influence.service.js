...
export async function createInfluenceNode(input) {
...
export async function getInfluenceNodes(profileId) {
    return db.orm.public.InfluenceNode
        .where({ profileId })
        .all();
}
export async function getInfluenceNodeById(id) {
    return db.orm.public.InfluenceNode.first({
        id,
    });
}
export async function updateInfluenceNode(id, data) {
    const existing = await getInfluenceNodeById(id);
    if (!existing) {
...
export async function deleteInfluenceNode(id) {
    const existing = await getInfluenceNodeById(id);
    if (!existing) {
        throw new Error("Influence node not found.");
    }
...
/* ---------------- CONNECTIONS ---------------- */
export async function createInfluenceConnection(input) {
    const fromNode = await db.orm.public.InfluenceNode.first({
        id: input.fromNodeId,
    });
    const toNode = await db.orm.public.InfluenceNode.first({
        id: input.toNodeId,
    });
    if (!fromNode || !toNode) {
        throw new Error("One or both influence nodes were not found.");
    }
    if (fromNode.profileId !== toNode.profileId) {
        throw new Error("Nodes must belong to the same monitoring profile.");
    }
    const existing = await db.orm.public.InfluenceConnection.first({
...
export async function getInfluenceConnections(profileId) {
    const nodes = await getInfluenceNodes(profileId);
    const nodeIds = nodes.map((node) => node.id);
    const connections = await db.orm.public.InfluenceConnection.all();
    return connections.filter((connection) => nodeIds.includes(connection.fromNodeId) &&
        nodeIds.includes(connection.toNodeId));
}
export async function deleteInfluenceConnection(id) {
    const existing = await db.orm.public.InfluenceConnection.first({
        id,
    });
    if (!existing) {
        throw new Error("Influence connection not found.");
    }
    return db.orm.public.InfluenceConnection
        .where({ id })
        .delete();
}
export async function computeInfluenceNetworkWithML(profileId) {
    const profile = await db.orm.public.MonitoringProfile.first({
        id: profileId,
    });
    if (!profile) {
    ...
    if (nodes.length < 2) {
        ...
        for (const n of nodes) {
            if (n.username)
                createdNodesMap.set(n.username.toLowerCase(), n.id);
        }
