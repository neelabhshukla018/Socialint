...
export async function analyzeTrendsAndTopicsWithML(profileId: number) {
  const profile = await db.orm.public.MonitoringProfile.first({
    id: profileId,
  });
  ...
  let topics: import("./mlClient.service.js").TopicClusterResult[] = [];
  if (validPosts.length >= 2) {
    try {
      topics = await mlClient.extractTopics(
        validPosts.map((p) => ({ id: String(p.id), text: p.content || "" })),
        Math.min(5, Math.max(2, Math.floor(validPosts.length / 2)))
      );
    } catch (err) {
  ...
  const hashtagMap = new Map<string, { count: number; engagements: number[]; timestamps: string[] }>();
  ...
  let scoredTrends: import("./mlClient.service.js").TrendScoreResult[] = [];
