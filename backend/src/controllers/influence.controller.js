import { createInfluenceNode, getInfluenceNodes, getInfluenceNodeById, updateInfluenceNode, deleteInfluenceNode, createInfluenceConnection, getInfluenceConnections, deleteInfluenceConnection, computeInfluenceNetworkWithML, } from "../services/influence.service.js";
/* =========================================================
   INFLUENCE NODES
   ========================================================= */
/**
 ...
 */
export async function createInfluenceNodeController(req, res) {
    try {
        const { profileId, name, username, category, influenceScore, xPosition, yPosition, } = req.body;
        if (!profileId) {
        ...
        const node = await createInfluenceNode({
            ...
            username,
            category,
            influenceScore: influenceScore !== undefined
                ? Number(influenceScore)
                : undefined,
            xPosition: xPosition !== undefined
                ? Number(xPosition)
                : undefined,
        ...
        console.error("Create influence node error:", error);
 ...
 */
export async function getInfluenceNodesController(req, res) {
    try {
        ...
        console.error("Get influence nodes error:", error);
        return res.status(500).json({
            success: false,
 ...
 */
export async function getInfluenceNodeController(req, res) {
    try {
        ...
        console.error("Get influence node error:", error);
        return res.status(500).json({
            success: false,
 ...
 */
export async function updateInfluenceNodeController(req, res) {
    try {
        const id = Number(req.params.id);
        if (!id || Number.isNaN(id)) {
            return res.status(400).json({
        ...
        const { name, username, category, influenceScore, xPosition, yPosition, } = req.body;
        const data = {};
        if (name !== undefined) {
            data.name = name;
        }
        ...
        console.error("Update influence node error:", error);
        return res.status(500).json({
            success: false,
            message: error instanceof Error
                ? error.message
 ...
 */
export async function deleteInfluenceNodeController(req, res) {
    try {
        const id = Number(req.params.id);
        if (!id || Number.isNaN(id)) {
            return res.status(400).json({
        ...
        console.error("Delete influence node error:", error);
        return res.status(500).json({
            success: false,
 ...
 */
export async function createInfluenceConnectionController(req, res) {
    try {
        const { fromNodeId, toNodeId, strength, interactions, } = req.body;
        ...
        const connection = await createInfluenceConnection({
        ...
        console.error("Create influence connection error:", error);
        return res.status(500).json({
            success: false,
            message: error instanceof Error
                ? error.message
 ...
 */
export async function getInfluenceConnectionsController(req, res) {
    try {
        const profileId = Number(req.query.profileId);
        if (!profileId || Number.isNaN(profileId)) {
            return res.status(400).json({
        ...
        const connections = await getInfluenceConnections(profileId);
        return res.status(200).json({
            success: true,
        ...
        console.error("Get influence connections error:", error);
        return res.status(500).json({
            success: false,
            message: "Failed to fetch influence connections.",
        });
    }
}
/**
 ...
 */
export async function deleteInfluenceConnectionController(req, res) {
    try {
        const id = Number(req.params.id);
        if (!id || Number.isNaN(id)) {
            return res.status(400).json({
        ...
        return res.status(200).json({
            success: true,
            message: "Influence connection deleted successfully.",
        });
    }
    catch (error) {
        console.error("Delete influence connection error:", error);
        return res.status(500).json({
            success: false,
            message: "Failed to delete influence connection.",
        });
    }
}
/**
 ...
 */
export async function analyzeInfluenceNetworkController(req, res) {
    try {
