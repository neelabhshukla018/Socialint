 ...
 */
export async function ingestPostsController(req, res) {
    try {
        ...
        const result = await dataIngestionService.ingestAndSaveBatch(profileId, postsArray);
        ...
        return res.status(500).json({
            success: false,
            message: error instanceof Error ? error.message : "Failed to ingest posts.",
        });
    }
}
/**
 ...
 */
export async function previewIngestionController(req, res) {
    try {
        const rawPosts = req.body.posts || req.body;
        const postsArray = Array.isArray(rawPosts) ? rawPosts : [rawPosts];
        if (!postsArray.length) {
        ...
        return res.status(500).json({
            success: false,
            message: error instanceof Error ? error.message : "Failed to preview ingestion.",
        });
    }
}
