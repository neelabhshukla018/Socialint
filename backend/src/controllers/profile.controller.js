import { createProfile, getProfiles, getProfileById, updateProfile, setActiveProfile, deleteProfile, } from "../services/profile.service.js";
export async function createProfileController(req, res) {
    try {
        const { clerkId, email, name, username, profileType, profileName, identifier, description, } = req.body;
        if (!clerkId ||
            !email ||
            !profileType ||
            !profileName ||
        ...
        if (!["PERSON", "BRAND", "CAMPAIGN"].includes(profileType)) {
            return res.status(400).json({
                success: false,
...
export async function activateProfileController(req, res) {
    try {
        const id = Number(req.params.id);
