import { getDataSources, getDataSourceById, connectDataSource, updateDataSource, disconnectDataSource, deleteDataSource, } from "../services/dataSource.service.js";
/**
 * GET /api/data-sources?profileId=1
 */
export async function getDataSourcesController(req, res) {
    try {
 ...
 */
export async function getDataSourceController(req, res) {
    try {
 ...
 */
export async function connectDataSourceController(req, res) {
    try {
        const { profileId, platform, username, profileUrl, externalId, } = req.body;
        if (!profileId) {
            return res.status(400).json({
                success: false,
                message: "profileId is required.",
 ...
 */
export async function updateDataSourceController(req, res) {
    try {
        const id = Number(req.params.id);
        if (!id || Number.isNaN(id)) {
            return res.status(400).json({
 ...
 */
export async function disconnectDataSourceController(req, res) {
    try {
        const id = Number(req.params.id);
        if (!id || Number.isNaN(id)) {
            return res.status(400).json({
 ...
 */
export async function deleteDataSourceController(req, res) {
    try {
        const id = Number(req.params.id);
        if (!id || Number.isNaN(id)) {
            return res.status(400).json({
