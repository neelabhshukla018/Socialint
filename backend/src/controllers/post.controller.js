import { createPost, getPosts, getPostById, updatePost, deletePost, enrichPostsWithML, } from "../services/post.service.js";
export async function createPostController(req, res) {
    try {
        const { profileId, sourceId, externalId, authorName, authorHandle, content, url, postType, likes, comments, shares, views, sentiment, sentimentScore, publishedAt, } = req.body;
        if (!profileId) {
        ...
        if (postType && !allowedPostTypes.includes(postType)) {
            return res.status(400).json({
                success: false,
                message: "Invalid postType. Use POST, REPLY, COMMENT, VIDEO or ARTICLE.",
            });
        }
...
export async function enrichPostsController(req, res) {
    try {
